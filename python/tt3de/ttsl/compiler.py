...
PIXELVAR_TT_NORMAL: str = "tt_Normal"
...
class TTSLCompilerContext:
    ...
    def always_present_variables(cls) -> Dict[str, IRType]:
        ...
        ``tt_Normal`` as ``glm.vec3`` (interpolated geometric normal in view space;
        ``(0, 0, 1)`` matches ``PixInfo`` initialization until the material bridge
        overwrites the register), ``tt_ViewPos`` as ``glm.vec3`` (fragment position in
        view space; ``(0, 0, 0)`` when not supplied), and ``tt_PrimitiveID`` as ``int``:
        per-pixel index of the depth-winning primitive,
...
class RegisterAllocatorPass(CompilationPass):
    def run(self) -> RegisterAllocationResult:
        ...
        for var_name, temp in named_variables.items():
            ...
            elif var_name == PIXELVAR_TT_NORMAL:
                # Pin to ``regs.v3_[2]`` (see ``ShaderInputBinding::normal_v3_reg``); ``0``/``1``
                # are the UV ``vec2``→``vec3`` bridge.
                assert (
                    temp.ty == IRType.V3
                ), "tt_Normal must be IRType.V3 (declared in PIXEL_VARIABLES_STR_TYPE)"
                idx = 2
                allocated_registers.setdefault(IRType.V3, set()).add(2)
            elif var_name == PIXELVAR_TT_VIEW_POS:
                # Pin to ``regs.v3_[3]`` (see ``ShaderInputBinding::view_pos_v3_reg``).
                assert (
                    temp.ty == IRType.V3
...
def apply_engine_uniform_register_defaults(reg_settings: RegisterSettings) -> None:
    ...
    ``tt_Normal`` defaults to ``(0, 0, 1)`` for standalone VM runs (``PixInfo`` default);
    ``ShaderMaterial`` writes ``PixInfo::normal`` each pixel (``ShaderInputBinding::normal_v3_reg``,
    default ``2``).
    ...
    if PIXELVAR_TT_NORMAL in names:
        # Matches ``PixInfo::default`` / ``new`` normal until the material bridge writes
        # ``PixInfo::normal`` each pixel.
        reg_settings.set_variable(PIXELVAR_TT_NORMAL, glm.vec3(0.0, 0.0, 1.0))
