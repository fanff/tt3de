...
mod test_front_facing_winding {
    ...
    use super::{oriented_area_2d_xy, triangle_front_facing_submission_order_xy, DrawBuffer};
    ...
    fn flip_y_default_buffer_negates_area_sign_vs_no_flip() {
        ...
        assert_ne!(
            triangle_front_facing_submission_order_xy(
                sa_n.x, sa_n.y, sb_n.x, sb_n.y, sc_n.x, sc_n.y
            ),
            triangle_front_facing_submission_order_xy(
                sa_f.x, sa_f.y, sb_f.x, sb_f.y, sc_f.x, sc_f.y
            )
        );
    }
...
impl<const L: usize, DEPTHACC: Number> DrawBuffer<L, DEPTHACC> {
    ...
    pub fn cell_center_to_ndc(&self, col: usize, row: usize) -> Vec2 {
        ...
        let s = self.scale;
        vec2((screen.x / h.x - 1.0) / s.x, (screen.y / h.y - 1.0) / s.y)
    }
