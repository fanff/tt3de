...
#[derive(Debug, Clone)]
pub struct SmallTriangleBuffer<const INNER_SIZE: usize> {
    pub content: [[Vec4; 3]; INNER_SIZE],
    pub uvs: [[Vec2; 3]; INNER_SIZE],
    /// Per-corner view-space positions (interpolated through clip); multiply by NDC `w` at emit time for `Vertex::view_pos`.
    pub view_pos: [[Vec3; 3]; INNER_SIZE],
    pub count: usize,
}

impl<const INNER_SIZE: usize> Default for SmallTriangleBuffer<INNER_SIZE> {
    fn default() -> Self {
        Self::new()
    }
}

impl<const INNER_SIZE: usize> SmallTriangleBuffer<INNER_SIZE> {
    pub fn new() -> Self {
        Self {
            content: [[Vec4::zeros(); 3]; INNER_SIZE],
            uvs: [[Vec2::zeros(); 3]; INNER_SIZE],
            view_pos: [[Vec3::zeros(); 3]; INNER_SIZE],
            count: 0,
        }
    }
    ...
    pub fn push_vec4(
        ...
        self.view_pos[self.count] = [*vpa, *vpb, *vpc];
        self.count += 1;
    }

    /// Clip-space triangle with UV only; view-space corner attributes default to zero (2D / generic clip paths).
    pub fn push_vec4_no_view(
        &mut self,
        a: Vec4,
        b: Vec4,
        c: Vec4,
        ...
        let z = Vec3::zeros();
        self.push_vec4(a, b, c, (uva, uvb, uvc), (&z, &z, &z));
    }
    ...
    pub fn iter(&self) -> impl Iterator<Item = (&[Vec4; 3], &[Vec2; 3], &[Vec3; 3])> + '_ {
        (0..self.count).map(|i| (&self.content[i], &self.uvs[i], &self.view_pos[i]))
    }
}
...
pub fn half_clip_triangle_to_plane<const INNER_SIZE: usize>(
    ...
    output_buffer: &mut SmallTriangleBuffer<INNER_SIZE>,
) {
    ...
    if inside_pa || inside_pb || inside_pc {
        output_buffer.push_vec4_no_view(*pa, *pb, *pc, uv_triplet);
    }
}
...
pub fn clip_triangle_to_plane<const INNER_SIZE: usize>(
    ...
    if inside_pa && inside_pb && inside_pc {
        ...
        if inside_points.len() == 1 && outside_points.len() == 2 {
            ...
            output_buffer.push_vec4_no_view(*inside_points[0], p1, p2, (inside_uv[0], &uv1, &uv2));
            ...
            output_buffer.push_vec4_no_view(
                *inside_points[0],
                *inside_points[1],
                p1,
                (inside_uv[0], inside_uv[1], &uv1),
            );
            output_buffer.push_vec4_no_view(*inside_points[1], p1, p2, (inside_uv[1], &uv1, &uv2));
        }
    }
}
...
/// use tt3de_core::primitiv_building::triangle_clipping::{clip_triangle_to_clip_space, SmallTriangleBuffer};
///
...
pub fn clip_triangle_to_clip_space(
    ...
    let mut buffer1: SmallTriangleBuffer<12> = SmallTriangleBuffer::new();
    let mut buffer2: SmallTriangleBuffer<12> = SmallTriangleBuffer::new();
    buffer1.push_vec4_no_view(*pa, *pb, *pc, (uva, uvb, uvc));
    ...
    for (plane, method) in planes.iter() {
        ...
        for (vertex_triplet, uv_triplet, _view_triplet) in input_buffer.iter() {
    ...
    for (triangle, uvs, _view) in input_buffer.iter() {
        let cpa: Vec4 = triangle[0];
        let cpb: Vec4 = triangle[1];
        let cpc: Vec4 = triangle[2];

        output_buffer.push_vec4_no_view(cpa, cpb, cpc, (&uvs[0], &uvs[1], &uvs[2]));
    }
}
...
mod tests_clip_triangle_to_space {
    ...
    fn test_clip_triangle_within_clip_space() {
        ...
        let mut output_buffer = SmallTriangleBuffer::new();
        let uvs = (
            &Vec2::new(0.0, 0.0),
            &Vec2::new(1.0, 0.0),
            &Vec2::new(0.0, 1.0),
    ...
    fn test_clip_triangle_in_4() {
        ...
        assert_eq_vec3!(
            clipped_triangle[2],
            Vec4::new(0.3333333, 1.0, 0.0, 1.0),
            VERTEX_EPS
        );
...
/// Clips a triangle to the view frustum in clip space, but only in X and Y.
///
/// This function clips the triangle against the four planes:
/// -x ≤ w, x ≤ w, -y ≤ w, y ≤ w
///
/// It does **not** clip in Z; you are expected to handle near/far clipping
/// separately if needed.
///
/// Uses the Sutherland-Hodgman clipping algorithm similarly to
/// `clip_triangle_to_clip_space`, but restricted to X/Y planes.
///
/// # Parameters
///
/// - `pa`: The first vertex of the triangle in clip space.
...
///
/// - `output_buffer`: The buffer to store the resulting clipped triangles.
pub fn clip_triangle_to_clip_space_xy(
    pa: &Vec4,
    pb: &Vec4,
    pc: &Vec4,
    (uva, uvb, uvc): (&Vec2, &Vec2, &Vec2),
    output_buffer: &mut SmallTriangleBuffer<8>,
) {
    // Only X and Y planes: left, right, bottom, top
    let planes = [
        (Vec4::new(-1.0, 0.0, 0.0, 1.0), 0), // left
        (Vec4::new(1.0, 0.0, 0.0, 1.0), 0),  // right
        (Vec4::new(0.0, -1.0, 0.0, 1.0), 0), // bottom
        (Vec4::new(0.0, 1.0, 0.0, 1.0), 0),  // top
    ];

    output_buffer.clear();

    // internal buffers – capacity adjusted to 8
    let mut buffer1: SmallTriangleBuffer<8> = SmallTriangleBuffer::new();
    let mut buffer2: SmallTriangleBuffer<8> = SmallTriangleBuffer::new();
    buffer1.push_vec4_no_view(*pa, *pb, *pc, (uva, uvb, uvc));

    let mut input_buffer = &mut buffer1;
    let mut output_buffer_temp = &mut buffer2;

    // clip the triangle to each XY plane
    for (plane, method) in planes.iter() {
        output_buffer_temp.clear();
    ...
    // copy the clipped triangles to the final output buffer
    for (triangle, uvs, _view) in input_buffer.iter() {
        let cpa: Vec4 = triangle[0];
        let cpb: Vec4 = triangle[1];
        let cpc: Vec4 = triangle[2];
