...
use tt3de_core::primitivbuffer::primitivbuffer::{
    PointInfo, PrimitivReferences, PrimitiveBuffer, PrimitiveElements,
};
...
use tt3de_core::raster::vertex::Vertex;
...
fn into_dict(_py: Python, primitive_ref: &PrimitivReferences, dict: &Bound<PyDict>) {
    ...
    dict.set_item("primitive_id", primitive_ref.primitive_id)
        .unwrap();
    dict.set_item("transparent", primitive_ref.transparent)
        .unwrap();
}
