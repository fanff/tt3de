...
**Implicit inputs (per cell)** — `tt_FragCoord`, `tt_FragPos`, `tt_Normal`, `tt_ViewPos`, `tt_TexCoord0`, `tt_TexCoord1`, `tt_FrontFacing`, `tt_FragDepth`, `tt_LineCoord`, `tt_PointCoord`, `tt_PrimitiveID` are built into the compiler and filled by the material bridge each shaded cell; they are **not** `globals_dict` keys.
