# -*- coding: utf-8 -*-
"""Generate the Rust SSA fixtures used by the TTSL execution benchmark.

The TTSL front-end lives in Python while Cranelift lives in ``tt3de-core``.
Compiling demo shaders at benchmark time would put Python on the ``cargo bench``
path, so the SSA snapshot and register seeds are compiled here and emitted as
committed files.
...
SSA_DIR = OUTPUT_PATH.parent / "ssa"
...
FIXTURE_SPECS: Sequence[FixtureSpec] = (
    ...
    FixtureSpec(
        ...
        uniforms={
            ...
            "u_g0": ModuleAttr("GLYPH_G0"),
            "u_g1": ModuleAttr("GLYPH_G1"),
            "u_g2": ModuleAttr("GLYPH_G2"),
            "u_g3": ModuleAttr("GLYPH_G3"),
            "u_g4": ModuleAttr("GLYPH_G4"),
            "u_g5": ModuleAttr("GLYPH_G5"),
            "u_g6": ModuleAttr("GLYPH_G6"),
            "u_g7": ModuleAttr("GLYPH_G7"),
            "u_g8": ModuleAttr("GLYPH_G8"),
            "u_g9": ModuleAttr("GLYPH_G9"),
        },
        varyings={
            PIXELVAR_TT_FRAGCOORD: glm.vec2(0.37, 0.61),
...
def build_fixtures() -> List[CompiledFixture]:
    """Compile every demo shader and collect its SSA snapshot plus seeded registers."""
    ...
    for spec in FIXTURE_SPECS:
        ...
        if not result.ok:
            raise RuntimeError(
                f"{spec.name} compilation failed:\n{result.traceback_text}"
            )
        assert result.register_settings is not None
        assert result.ssa_module is not None
        _seed(result.register_settings, spec.uniforms, module)
        _seed(result.register_settings, spec.varyings, module)
        out.append(
            CompiledFixture(
                spec=spec,
                registers=result.register_settings.get_register_list(),
                ssa_json=json.dumps(result.ssa_module, indent=2) + "\n",
            )
        )
    return out
    ...
    for fixture in fixtures:
        ...
        lines.extend(
            [
                ...
                f'        ssa_json: include_str!("ssa/{spec.name}.json"),',
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    rendered = format_rust(render_rust(fixtures))
    ssa_files = {fixture.spec.name: fixture.ssa_json for fixture in fixtures}
    ...
    if args.check:
        ...
        for name, text in ssa_files.items():
            path = SSA_DIR / f"{name}.json"
            if not path.is_file() or path.read_text(encoding="utf-8") != text:
                stale = True
                break
        if stale:
            print(
                f"{OUTPUT_PATH.relative_to(REPO_ROOT)} or {SSA_DIR.relative_to(REPO_ROOT)} "
                "is stale; run scripts/gen_ttsl_bench_fixtures.py",
                file=sys.stderr,
            )
            return 1
        return 0
    ...
    for name, text in ssa_files.items():
        (SSA_DIR / f"{name}.json").write_text(text, encoding="utf-8")
    OUTPUT_PATH.write_text(rendered, encoding="utf-8")
    print(f"wrote {OUTPUT_PATH.relative_to(REPO_ROOT)}")
    print(f"wrote {len(ssa_files)} SSA snapshots under {SSA_DIR.relative_to(REPO_ROOT)}")
    return 0
