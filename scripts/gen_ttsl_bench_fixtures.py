...
Python on the ``cargo bench`` path, so the bytecode, SSA snapshot, and register
seeds are compiled here and emitted as committed files.
...
def build_fixtures() -> List[CompiledFixture]:
    ...
    for spec in FIXTURE_SPECS:
        ...
        assert result.byte_array is not None
...
def render_rust(fixtures: Sequence[CompiledFixture]) -> str:
    lines: List[str] = [
        ...
        "//",
        "// Bytecode, SSA JSON, and register seeds compiled from the TTSL demo shaders under demos/.",
        "",
        ...
        "    /// Post-SSA CFG dumped after `PassSSARenamer` (inputs include VM register ids).",
