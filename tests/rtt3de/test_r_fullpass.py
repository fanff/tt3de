...
class Test_PrimitivBuilding(unittest.TestCase):
    ...
    def test_one_line3D_inside_frustrum(self):
        ...
        transform_pack.set_projection_matrix(glm.perspectiveFovLH_ZO(glm.radians(90), 512, 512, 1.0, 10.0))
        ...
        assertLine3DPrimitiveKeyEqual(prim0, {
            "primitive_id":0,
            "geometry_id":1,
            "node_id":node_id,
        ...
        self.assertGreater(prim0["pb"]["depth"], prim0["pa"]["depth"]) # pb is after pa; clearly
    ...
    def test_one_line3D_cutting_backplane(self):
        ...
        self.assertEqual(vertex_buffer.add_vertex(0.0,0.5,100.0),1) # too far away from the back of the frustrum plane; will be clipped.
        ...
        self.assertGreater(prim0["pb"]["depth"], prim0["pa"]["depth"]) # pb is after pa; clearly
        self.assertAlmostEqual( prim0["pb"]["depth"], 1.0) # pb is the clipped point. at the back of the frustum
    ...
    def test_simple_one_triangle(self):
        ...
        material_id = 1
        uv_index = 2
        self.assertEqual(geometry_buffer.add_polygon_3d(0, 1, node_id, material_id,uv_index),1)
    ...
    def test_simple_two_triangle(self):
        ...
        self.assertEqual( geometry_buffer.add_polygon_fan_3d(0, 2, node_id, material_id,2),
                            1)
    ...
    def test_points_with_camera_left_right(self):
        ...
        prim1 = primitive_buffer.get_primitive(1)
        assertPointPrimitiveAlmostEqual(prim1,{
            "primitive_id":1,
            "geometry_id":2,
            "node_id":node_id,
            "material_id":23,
    ...
    def test_points_with_camera_top_down(self):
        ...
        vertices = [
            point_inside_frustrum,
            point_inside_frustrum+(camera.up_vector()*(-.3)),
            point_inside_frustrum+(camera.up_vector()*(-.1)),
            point_inside_frustrum+(camera.up_vector()*(.1)),
            point_inside_frustrum+(camera.up_vector()*(.3)) #
        ]
...
class Test3DLineClippingCases(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.geometry_buffer.add_point(0, 0, node_id=0, material_id=0) # this is the geomid default and MUST be the background.
    ...
    def test_one_line3D_clipping_backplane(self):
        ...
        # setup a point away from the camera; controlling clipping
        self.vertex_buffer.add_vertex(0.0, 0.5, 100.0) # too far away from the back of the frustrum plane; will be clipped.
    ...
    def test_one_line3D_clipping_nearplane(self):
        ...
        self.vertex_buffer.add_vertex(0.0, 0.5, -4.1) # too close to the near plane; will be clipped.

        # now add the line
        material_id = 1
        self.assertEqual(self.geometry_buffer.add_line3d(0, self.node_id, material_id, 0), 1)
    ...
    def test_one_line3D_clipping_leftplane(self):
        ...
        self.vertex_buffer.add_vertex(-100.0, 0.5, 5.0) # too far to the left of the frustum plane; will be clipped.
        ...
        self.assertLessEqual(prim0["pb"]["row"], 256) # is a little bellow pa; because y is positive
    ...
    def test_one_line3D_clipping_rightplane(self):
        ...
        self.vertex_buffer.add_vertex(100.0, 0.5, 5.0) # too far to the right of the frustum plane; will be clipped.
        ...
        self.assertEqual(prim0["pb"]["col"], 511) # pb is at the Right of the frustum plane
    ...
    def test_one_line3D_clipping_topplane(self):
        ...
        self.vertex_buffer.add_vertex(0.0, -100.0, 5.0) # too far above the frustum plane; will be clipped.
        ...
        self.assertEqual(prim0["pb"]["row"], 511) # pb the is at the top of the frustum plane
    ...
    def test_one_line3D_clipping_bottomplane(self):
        ...
        self.vertex_buffer.add_vertex(0.0, 100.0, 5.0) # too far below the frustum plane; will be clipped.
        ...
        self.assertEqual(prim0["pb"]["row"], 0) # pb the is at the bottom of the frustum plane
        self.assertEqual(prim0["pb"]["col"], 256) # in the middle from left right
        self.assertLess(prim0["pb"]["depth"], 1.0) #
        self.assertGreater(prim0["pb"]["col"], 0.0) #
