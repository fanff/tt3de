...
class TestFaceforward(unittest.TestCase):
    ...
    def test_triangle_clipping_project(self):
        ...
        for pzidx, pz in enumerate(range(-10, 40)):
            v1 = glm.vec3(2, -2, 0.5 * pz - 10.1)
            v2 = glm.vec3(-2, -2, 0.5 * pz - 10.1)
            apoint3 = glm.vec3(0.0, -2.0, 0.5 * pz + 0.1)
            # tridet = glm.determinant(glm.mat3x3(v1,v2,apoint3))
            tri_dot = glm.dot(calculate_normal(v1, v2, apoint3), glm.vec3(0, 0, 1))

            triangle_to_draw = filter_clip(
                v1, v2, apoint3, near_plane + 0.1, m, p, viewport
            )
            ...
            for tri, normal in triangle_to_draw:
                ...
                pp1_scaled_pil = pp1_scaled_pil * glm.vec3(
                    pil_output_width, pil_output_height, 1.0
                )
                pp2_scaled_pil = pp2_scaled_pil * glm.vec3(
                    pil_output_width, pil_output_height, 1.0
                )
                pp3_scaled_pil = pp3_scaled_pil * glm.vec3(
                    pil_output_width, pil_output_height, 1.0
                )

                triangle = [
                    tuple(pp1_scaled_pil.xy),
