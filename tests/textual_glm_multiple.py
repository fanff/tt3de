...
class GLMTester(TT3DView):
    ...
    def update_step(self, timediff):
        ...
        atransform = glm.scale(glm.vec2(2, 2)) * glm.translate(glm.vec2(-0.5, -0.5))

        for idx, e in enumerate(self.root2Dnode.elements):
            e.local_transform = (
                glm.translate(glm.vec2(2 * (idx), 0.0))
                * glm.rotate(rot * (idx + 1))
                * atransform
            )
        self.root2Dnode.local_transform = glm.translate(glm.vec2(0.5, 0.5)) * glm.scale(
            glm.vec2(0.2, 0.2)
        )
