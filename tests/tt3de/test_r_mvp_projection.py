...
class TestMVP_Projection(unittest.TestCase):
    ...
    def test_simple_central_points(self):
        ...
        transform_buffer.set_view_matrix_3d(glm.lookAt(glm.vec3(0, 0, 3), glm.vec3(0, 0,-3), glm.vec3(0, 1, 0)))
        
        vertex_buffer = VertexBufferPy()
        self.assertEqual(vertex_buffer.get_vertex_count(), 0)

        vertices = [
            Point3D(0, 0, 1.0), # this point is in the middle of the screen, far enough from the camera
            Point3D(0.0, 0.0, 1.8), # this point is closer to the camera ; still in the frustum
            Point3D(0.0, 0.0, 2.9), # this is the point just in front of it the camera; not is the frustum, too close
            Point3D(0.0, 0.0, 5.0), # this is the point far in the back; not in the frustum
        ]
