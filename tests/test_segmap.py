...
def method_1(drawing_buffer,style_array):
    ...
    segs = []
    for idx, (fr,fg,fb,br,bg,bb,g1,g2) in enumerate(drawing_buffer.canvas_to_list()):
        asegment = Segment(" ",Style(color=Color.from_triplet(ColorTriplet(fr,fg,fb)),
...
def method_2(drawing_buffer,big_buffer,allchars):
    ...
    for (fr,fg,fb,br,bg,bb,g1,g2) in (drawing_buffer.canvas_to_list()):
        segid = (fb//32 + 8 * ( fg//32 + 8*(fr//32 + 8 * ( bb//32 + 8*( bg//32 + 8 * (  br//32  )  )  )   )) )
                ...
                for bi in range(8):
                    for bj in range(8):
                        for bk in range(8):
                            ...
                            big_buffer.append(
                                                          Style(color= Color.from_triplet(ColorTriplet(i,j,k)),
                                                                bgcolor= Color.from_triplet(ColorTriplet(bi,bj,bk)) ))
