...
if __name__ == "__main__":
    ...
    native_py_duration = timeit.timeit(
        stmt="collections.deque(mesh.draw(camera, WIDTH,HEIGHT), maxlen=0)",
        number=itercount,
        setup=setupstr,
    )
    ...
    glmduration = timeit.timeit(
        stmt="collections.deque(glmMesh.draw(glmcam, WIDTH,HEIGHT), maxlen=0)",
        number=itercount,
        setup=setupstr,
    )
