...
if __name__ == "__main__":
    ...
    res = timeit.timeit(
        stmt="list(mesh.proj_vertices(camera,200,200))",
        number=itercount,
        setup="from __main__ import mesh,camera",
    )
    print(f"{(res/itercount)*1000:.5f} ms per iteration")

    glmMesh.proj_vertices(glmcam, perspective_matrix, 200, 200)
    res = timeit.timeit(
        stmt="list(glmMesh.proj_vertices(glmcam,perspective_matrix,200,200))",
        number=itercount,
        setup="from __main__ import glmMesh,glmcam,perspective_matrix",
    )
    print(f"{(res/itercount)*1000:.5f} ms per iteration")
