...
from tt3de.richtexture import ImageTexture, RenderContext, StaticTexture, get_cube_vertices
from tt3de.textual_widget import Cwr
from tt3de.tt3de import FPSCamera, Line3D, Mesh3D, PPoint2D, Point3D, PointElem, Triangle3D, load_bmp
...
proc_point2d()
    """
    res= timeit.timeit(stmt=stm, number=100000,setup="from __main__ import proc_point2d")
    print(res)

    stm = """
cproc_point2d()
    """
    res= timeit.timeit(stmt=stm, number=100000,setup="from __main__ import cproc_point2d")
    print(res)


    res= timeit.timeit(stmt="glmproc_point2d()", number=100000,setup="from __main__ import glmproc_point2d")
    print(res)
...
proc_point3d()
    """
    res= timeit.timeit(stmt=stm, number=100000,setup="from __main__ import proc_point3d")
    print(res)

    stm = """
proc_cpoint3d()
    """
    res= timeit.timeit(stmt=stm, number=100000,setup="from __main__ import proc_cpoint3d")
    print(res)

    res= timeit.timeit(stmt="glmproc_point3d()", number=100000,setup="from __main__ import glmproc_point3d")
    print(res)
