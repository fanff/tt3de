...
class Test_CalcInternals(unittest.TestCase):
    def test_calculate_zvariation(self):
        ...
        triangle1 = mat3(vec3(1.2, 3.4, 1.0), vec3(5.2, 5.4, 1.0), vec3(8.2, 2.4, 1.0))

        # the same triangle with various Z
        triangle2 = mat3(
            vec3(1.2, 3.4, -2321.5), vec3(5.2, 5.4, -100.1), vec3(8.2, 2.4, -123.6)
        )
        ...
        self.assertEqual(
            pix_buff_to_string(pixel_buff_1, 10, 10, index_shift=1),
...
class Test_TrianglesBuffer(unittest.TestCase):
    ...
    def test_raster_triangle(self):
        ...
        tricount = 10000
        manytri = [
            glm.mat3(*generate_random_triangle(CW_option=False))
        ...
        if duration != 0.0:
            print(
                f"duration baking {duration*1000:.2f} ms ({tricount/duration} tri/sec)"
            )
